...
/*
 * UX_PLAN §3.3: the weak list, not the graph. A force-directed diagram of 190
 ...
 *
 * Read-only. No editing, no assessment — a state moves only through the mastery
 * gate, which is not reachable from this screen.
 */
...
export default async function SkillsPage() {
  ...
  const rows = await listSkillRows(session.user.id)
    ...
    return (
      <section className="flex flex-col gap-3">
        ...
        <p className="text-step-1 text-text-2 max-w-prose">
          The skill graph has no nodes yet. The tables exist — migration 0003 is applied —
          but the ~190-node seed is still pending a decision about per-node estimates that
          no source currently provides.
        </p>
        <p className="text-step-0 text-text-3 font-mono tracking-widest uppercase">
          Not seeded yet · M1 task a
        </p>
      </section>
    )
  }
  ...
  return (
    <section className="flex flex-col gap-10">
      <div className="flex flex-col gap-2">
        <h1 className="text-step-3 font-medium">Skills</h1>
        <p className="text-step-1 text-text-2 max-w-prose">
          {rows.length} nodes, weakest first. The graph view is deferred — it answers what
          a skill unblocks, which is not the question this screen is for.
        </p>
      </div>
