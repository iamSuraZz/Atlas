...
└── dsa        { arrays-hashing, two-pointers, sliding-window, stack, binary-search, linked-list, trees, tries, heap-priority-queue, backtracking, graphs, dp-1d, dp-2d, greedy, intervals, math-bit-manipulation }
...
> **AMENDED 2026-09-22 — M-DS ruling 6. The DSA subtree.**
>
> `dsa` joins ENGINEERING_CORE: 1 topic + 16 leaves, taking engineering from
> 190 to **207** and the whole graph to **375**. Its 17 prerequisite edges
> follow the NeetCode roadmap order, taking the graph from 136 to **153**.
>
> Decay class `RECALL_HEAVY` — patterns you have not touched in a month are
> patterns you cannot produce under interview pressure. The objective artifact
> is passing tests, which is the default policy, so nothing special is needed
> to make the mastery gate read it.
>
> Phase **E1**, and mapped to the DSA thread. Before this the thread drew from
> two nodes (`interview/dsa-patterns` and `interview/code-comprehension`) for
> 15% of every week, which is not a rotation — it is the same two drills.

> **AMENDED 2026-09-22 — M-DS task a. A second track joins the graph.**
>
> `docs/DATA_ML_TRACK.md` adds a Data & ML curriculum: **168 nodes** across five
> new categories, bringing the graph to **375** (207 engineering + 168 data).
> Prerequisite edges go from 79 to **153**; seven of the 74 new edges point back
> into existing engineering nodes, which is where prior experience already
...
> | MATH_STATS | 25 | 4 |
> | DATA_SCIENCE | 35 | 6 |
> | MACHINE_LEARNING | 65 | 10 |
> | DATA_ENGINEERING | 22 | 5 |
> | MLOPS | 21 | 4 |
> | **New** | **168** | **29** |
>
> The tree below is a **second** fenced block on purpose. `seed-skills.mjs`
> reads the first block and still sees exactly 207; `seed-track.mjs` reads
> `DATA_ML_TRACK.md` §9.1 and sees exactly 168. Neither can silently absorb the
> other's nodes.
>
> Every node also carries a `track` and a `phase` (§9.2 there), so the scheduler
> stops offering D7 material in week one. Two things this section does **not**
> model, recorded rather than fudged:
>
> - **D8's cross-listing.** §9.2 puts `nlp` in D8 "plus the existing
>   AI_ENGINEERING topics, cross-listed". `skill.phase` holds one value, so
>   those topics keep their primary phase `E4`. Reaching them from D8 means
>   activating E4 alongside D8 — a row in `active_phase`, not a schema change.
> - **Per-skill resource and project links.** §8 tags resources with phases and
>   §6 tags projects with a phase; neither names skills. `resource_skill` and
>   `build_project_skill` are therefore *derived* — every topic in the tagged
>   phase — and regenerated by the seeder rather than curated by hand.
